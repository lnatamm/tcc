...
export default function EditMetricModal({ open, onClose, onSuccess, metric }) {
  ...
  useEffect(() => {
    if (open && metric) {
      ...
      const idsMetricsArray = metric.ids_metrics ? metric.ids_metrics.split(',').map(id => parseInt(id.trim())) : [];
      setFormData({
        name: metric.name || '',
        ...
        aggregated: metric.aggregated || false,
        value: metric.value !== null && metric.value !== undefined ? metric.value.toString() : ''
      });
      setMetricSelections(idsMetricsArray);
  ...
  const fetchMetrics = async () => {
    try {
      ...
      // Only non-aggregated metrics can be used as parameters
      // And exclude the current metric from the list
      setAvailableMetrics(response.data.filter(m => !m.aggregated && m.id !== metric?.id));
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.put(`/metrics/${metric.id}`, metricPayload);

      onSuccess();
      handleClose();
    } catch (error) {
      console.error('Error updating metric:', error);
      setErrors({ submit: 'Error updating metric. Please try again.' });
    } finally {
      setLoading(false);
    }
  };
          ...
          {formData.aggregated && (
            <>
              <FormControl fullWidth>
                <InputLabel>Formula (optional)</InputLabel>
                <Select
